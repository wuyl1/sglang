...
struct WorkerServerInfo {
    kv_events: Option<WorkerKvEventsInfo>,
}

#[derive(Debug, Deserialize)]
struct WorkerKvEventsInfo {
    #[serde(default)]
    worker_generation: String,
}

async fn fetch_worker_generation(
    client: &reqwest::Client,
    worker_address: &str,
) -> Result<String, BridgeError> {
    let url = format!("{}/server_info", worker_address.trim_end_matches('/'));
    let response =
        client.get(url).send().await.map_err(|error| {
            BridgeError::Decode(format!("Worker identity request failed: {error}"))
        })?;
    if !response.status().is_success() {
        return Err(BridgeError::Decode(format!(
            "Worker identity request returned {}",
            response.status()
        )));
    }
    let info = response
        .json::<WorkerServerInfo>()
        .await
        .map_err(|error| BridgeError::Decode(format!("Worker identity decode failed: {error}")))?;
    let generation = info
        .kv_events
        .map(|events| events.worker_generation)
        .unwrap_or_default();
    if generation.is_empty() {
        return Err(BridgeError::Decode(
            "Worker /server_info has no KV generation".into(),
        ));
    }
    Ok(generation)
}
...
async fn recover_and_stream(
    ...
    let identity_client = reqwest::Client::builder()
        .timeout(WORKER_GENERATION_REQUEST_TIMEOUT)
        .build()
        .map_err(|error| BridgeError::Config(format!("build Worker identity client: {error}")))?;
    let mut generation_failures = 0_u32;
        ...
        let message = tokio::select! {
            ...
            _ = epoch_check.tick() => {
                ...
                if !worker_generation.is_empty() {
                    match fetch_worker_generation(&identity_client, &worker.worker_address).await {
                        Ok(current_generation) if current_generation == worker_generation => {
                            generation_failures = 0;
                        }
                        Ok(_) => {
                            return Err(BridgeError::Decode("Worker generation changed".into()));
                        }
                        Err(error) => {
                            generation_failures = generation_failures.saturating_add(1);
                            if generation_failures >= WORKER_GENERATION_FAILURE_LIMIT {
                                return Err(error);
                            }
                            warn!(
                                ...
                                "Worker generation heartbeat failed; retaining placement until retry"
                            );
                        }
                    }
                }
...
mod tests {
    ...
    #[tokio::test]
    async fn reads_worker_generation_from_server_info() {
        let app = Router::new().route(
            "/server_info",
            get(|| async {
                Json(serde_json::json!({
                    "kv_events": {"worker_generation": "generation-2"}
                }))
            }),
        );
        ...
        let generation = fetch_worker_generation(&client, &format!("http://{address}"))
            .await
            .unwrap();
