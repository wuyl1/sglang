...
上游把序号分配、事件应用和快照捕获放在同一个线程串行，因此 snapshot 内容与水位天然一致。Bridge 用水位衔接实时事件，重叠部分可以重复应用，水位之外不会遗漏。

这个方案依赖 apply 幂等：REPORT 整体替换 `(replica, tier, block)`、REVOKE 移除、CLEAR 清空，重复应用不改变结果。如果以后把 component mask 改成增量合并，就必须重新设计恢复流程。
...
Bridge 需要增加 ZMQ snapshot 客户端，并解析上游的 barrier 帧；我们只取 `barrier_seq` 当水位，不等待 barrier。PR 还没合、CI 未过，现在推动上游补齐字段成本最低。
...
Bridge 在内存中维护一个**已应用序号**，即已经确认写入 Indexer 的最大 seq。它用来丢弃重复事件、检测 seq gap，以及在 legacy 模式下检测 seq 回退。
...
- 有 epoch 时用 epoch 变化判断 Worker 重启；只有 legacy publisher 才把 seq 回退作为重启信号；
- Snapshot 获取失败时保留当前索引并退避重试，不清空、不退出；
...
队列满意味着 Bridge 自己丢了事件，按 gap 处理。队列容量按峰值事件率和最长恢复时间配置，并根据压测结果调整。
